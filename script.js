...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ========== Функции Hugging Face API ==========
    
    async function checkHuggingFaceAPI() {
        try {
            const testResponse = await queryHuggingFaceAPI("Тестовое сообщение");
            isHuggingFaceAvailable = true;
            console.log("Hugging Face API доступен");
        } catch (error) {
            isHuggingFaceAvailable = false;
            console.error("Hugging Face API недоступен:", error);
            
            // Для премиум пользователей показываем уведомление
            if (isPremium) {
                addBotMessage("Внимание: Премиум-функции временно недоступны. Используется локальная база знаний.");
            }
        }
    }

    function startAPICheckInterval() {
        apiCheckInterval = setInterval(checkHuggingFaceAPI, HF_API_CHECK_INTERVAL);
    }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 401) {
                    ...
                    throw new Error("Модель загружается, попробуйте позже");
            ...
            console.error("Ошибка запроса к Hugging Face API:", error);
            throw error; // Перебрасываем ошибку для обработки в generateBotResponse
        }
    }
    ...
    async function generateBotResponse(userMessage) {
        try {
            // Для премиум пользователей используем Hugging Face API, если он доступен
            if (isPremium && isHuggingFaceAvailable) {
                try {
                    const response = await queryHuggingFaceAPI(userMessage);
                    addBotMessage(response);
                    return;
                } catch (apiError) {
                    console.error("Ошибка API, используем локальный ответ:", apiError);
                    // Продолжаем с локальным ответом
                }
            }
            
            // Для бесплатных пользователей или при ошибке API используем локальные ответы
